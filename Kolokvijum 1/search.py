...
class AStarSearch(Search):
    def select_state(self, states):
        ...
        best_state = states[0]
        best_heuristic = best_state.get_cost() + best_state.get_current_cost() + best_state.avoid_sensors()

        for state in states:
            heuristic = state.get_cost() + state.get_current_cost() + state.avoid_sensors()
